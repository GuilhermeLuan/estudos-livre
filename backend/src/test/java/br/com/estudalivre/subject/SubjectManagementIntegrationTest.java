...
class SubjectManagementIntegrationTest {
    ...
    void ownerArchivesAndRestoresASubjectWithoutDeletingIt() throws Exception {
        ...
        mockMvc.perform(post("/api/subjects/{id}/archive", subjectId)
                        .with(user(principal))
                        .with(csrf()))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(post("/api/subjects/{id}/restore", subjectId)
                        .with(user(principal))
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.archived").value(false));
    ...
    void subjectsAreInvisibleAndUnmodifiableToOtherUsers() throws Exception {
        ...
        mockMvc.perform(put("/api/subjects/{id}", otherSubjectId)
                        .with(user(owner))
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Nome alterado\"}"))
                .andExpect(status().isNotFound());
        mockMvc.perform(post("/api/subjects/{id}/archive", otherSubjectId)
                        .with(user(owner))
                        .with(csrf()))
                .andExpect(status().isNotFound());
        mockMvc.perform(post("/api/subjects/{id}/restore", otherSubjectId)
                        .with(user(owner))
                        .with(csrf()))
                .andExpect(status().isNotFound());
    }
    ...
    private UUID createSubject(IdentityPrincipal principal, String name) throws Exception {
        String body = mockMvc.perform(post("/api/subjects")
                        .with(user(principal))
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"" + name + "\"}"))
